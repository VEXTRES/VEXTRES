<div class="bg-gray-50 rounded-lg shadow border border-x-gray-200 overflow-hidden"
    x-data="data()"
>
    <div class="grid grid-cols-3 divide-x divide-gray-200">
        ...
        <div class="col-span-1">
            <div class="bg-gray-100 h-16 flex items-center px-4">
                <img src="{{ auth()->user()->profile_photo_url}}" alt="{{auth()->user()->name}}" class="w-10 h-10 object-cover object-center">
            </div>

            <div class="bg-white h-14 flex-items-center px-4 py-2">
                <x-input type="text" class="w-full" placeholder="Busque un chat o inicie uno nuevo" wire:model.live="search"/>
            </div>

            <div class="h-[calc(100vh-10.5rem)] overflow-auto border-t border-gray-200 sm:inline-block">
                                ...
                                <li class="cursor-pointer" wire:click="open_chat_contact({{$contact}})">
                                    <div class="flex">
                                        <figure class="flex-shrink-0">
                                            <img class="h-12 w-12 rounded-full object-cover object-center" src="{{ $contact->user->profile_photo_url }}" alt="{{ $contact->name }}" >
                                        </figure>
                ...
                @else
                    @foreach ($this->chats as $chatItem)
                        <div wire:key="chats-{{ $chatItem->id }}" wire:click="open_chat({{ $chatItem }})"
                            class="flex items-center justify-between hover:bg-gray-100 cursor-pointer px-3 {{ $chat && $chat->id === $chatItem->id ? 'bg-gray-200' : 'bg-white' }}">
                            <figure>
                                <img class="h-12 w-12 rounded-full object-cover object-center" src="{{ $chatItem->image }}" alt="{{ $chatItem->name }}">
                            </figure>
                            ...
                            <div class="w-[calc(100%-4rem)] py-4 border-b border-gray-200">
                                <div class="flex justify-between items-center">
                                    <div>
                                        <p>{{ $chatItem->name }}</p>
                                        <p class="text-xs text-gray-700 mt-1 truncate">{{$chatItem->messages->last()->body}}</p>
                                    </div>

                                    <div class="text-right">
                                        <p class="text-xs">{{ $chatItem->last_message_at->format('d-m-y h:i A') }}</p>
        ...
        <div class="col-span-2" >
            ...
            @if ($contactChat || $chat)
                ...
                <div class="bg-gray-100 h-16 flex items-center px-3">
                    ...
                    <div class="ml-4">
                        <p class="text-gray-800">
                            {{$chat ? $chat->name : $contactChat->name}}
                        </p>

                        <p class=" text-xs" x-show="chat_id != typingChatId">
                            Online
                        </p>
                    </div>
                </div>
                ...
                <div class="h-[calc(100vh-11rem)] overflow-auto px-3 py-2"
                    x-data="{ scroll: () => {   $nextTick(()=>$el.scrollTo(0, $el.scrollHeight)); }}"
                    x-init="scroll()"
                    @scroll-bottom.window="scroll()"
                >
                    {{--Contenido Chat --}}
                    @foreach ($this->messages as $message)
                        <div class="flex mb-2 {{ $message->user_id == auth()->id() ? 'justify-end' : ''}}">
                            <div class="rounded px-3 py-2 {{ $message->user_id == auth()->id() ? 'bg-green-100' : 'bg-gray-200'}}">
                                <p class="text-sm">
                                    {{$message->body}}
                                </p>

                                <p class="text-xs mt-1 text-gray-600 {{ $message->user_id == auth()->id() ? 'text-right' : ''}}">
                                    {{$message->created_at->format('d-m-y h:i A')}}
                                    @if($message->user_id == auth()->id())
                                        <i class="fa-solid fa-check-double ml-2 {{ $message->is_read ? 'text-blue-500' : 'text-gray-600' }}"></i>
                                    @endif
                                </p>
                            </div>
                        </div>
                ...
                <form class="bg-gray-100 h-16 flex items-center px-4" wire:submit.prevent="sendMessage()">
                    <x-input type="text" class="flex-1" placeholder="Escriba un mensaje aquí" wire:model.live="bodyMessage"/>
                    <button class="flex-shrink-0 ml-4 text-2xl text-gray-700"><svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512"><path d="M307 34.8c-11.5 5.1-19 16.6-19 29.2v64H176C78.8 128 0 206.8 0 304C0 417.3 81.5 467.9 100.2 478.1c2.5 1.4 5.3 1.9 8.1 1.9c10.9 0 19.7-8.9 19.7-19.7c0-7.5-4.3-14.4-9.8-19.5C108.8 431.9 96 414.4 96 384c0-53 43-96 96-96h96v64c0 12.6 7.4 24.1 19 29.2s25 3 34.4-5.4l160-144c6.7-6.1 10.6-14.7 10.6-23.8s-3.8-17.7-10.6-23.8l-160-144c-9.4-8.5-22.9-10.6-34.4-5.4z"/></svg></button>
                </form>
            @else
                <div class="w-full h-full flex justify-center items-center">
                    <div>
                        ...
                        <h1 class="text-center text-gray-500 text-2xl mt-4">WhatsApp</h1>
                    </div>
                </div>
            @endif
        </div>
